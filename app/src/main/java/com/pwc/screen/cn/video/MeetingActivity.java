...
public class MeetingActivity extends BaseEngineEventHandlerActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        mRemoteUserViewWidth = (int) TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_DIP, 90, getResources().getDisplayMetrics());
        mRemoteUserViewHeight = (int) TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_DIP, 160, getResources().getDisplayMetrics());
        setupRtcEngine();
        initViews();
...
//        LogUtil.log.d(getApplicationContext().getExternalFilesDir(null).toString() + "/agorasdk.log");
    ...
    void initViews() {
        ...
        cameraEnabler.setOnCheckedChangeListener(new CompoundButton.OnCheckedChangeListener() {
            ...
            public void onCheckedChanged(CompoundButton compoundButton, boolean disablesCamera) {
                ...
                //Edited by Fox that when local user disabled video, set LocalVideo to background when no one is in the meeting
                //Set remotevideo to background when someone has joined the meeting.
                if (disablesCamera) {

                    if (mRemoteUserContainer != null && mRemoteUserContainer.getChildCount() > 0) {
                        View remoteView =  mRemoteUserContainer.getChildAt(0);
                        remoteView.findViewById(R.id.remote_user_voice_container).setVisibility(View.VISIBLE);
                    } else {
                    ...
                    if (mRemoteUserContainer != null && mRemoteUserContainer.getChildCount() > 0) {
                        View remoteView =  mRemoteUserContainer.getChildAt(0);
                        remoteView.findViewById(R.id.remote_user_voice_container).setVisibility(View.GONE);
                    } else {
    ...
    void setRemoteUserViewVisibility(boolean isVisible) {
        ...
        findViewById(R.id.user_remote_views).getLayoutParams().height =
                isVisible ? (int) TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_DIP, 160, getResources().getDisplayMetrics())
                        : 0;
        ...
        runOnUiThread(new Runnable() {
            ...
            public void run() {
                ...
                if (remoteUserView == null) {
                    ...
                    mRemoteUserContainer.addView(singleRemoteUser, new LinearLayout.LayoutParams(mRemoteUserViewWidth, mRemoteUserViewHeight));
                ...
                rtcEngine.enableVideo();
                //Edited by Fox: set local video to remote view and remote video to localview which is much bigger.
                int successCode = rtcEngine.setupRemoteVideo(new VideoCanvas(mLocalView, VideoCanvas.RENDER_MODE_ADAPTIVE, uid));
                ...
                rtcEngine.setupLocalVideo(new VideoCanvas(remoteView, VideoCanvas.RENDER_MODE_ADAPTIVE, uid));
    ...
    public void onUserOffline(final int uid) {
        ...
        runOnUiThread(new Runnable() {
            ...
            public void run() {
                ...
                //Edited by Fox: set local video back to localview
    ...
    public void onUserMuteVideo(final int uid, final boolean muted) {
        ...
        runOnUiThread(new Runnable() {
            @Override
            public void run() {
                //Edited by Fox that disable remote video (on localview) when remote user has disabled video
...
//                                : View.GONE);
//                remoteView.invalidate();
                if (CALLING_TYPE_VIDEO==mCallingType && muted) {
                    findViewById(R.id.user_local_voice_bg).setVisibility(View.VISIBLE);
        ...
        if(101==err){
            ...
            runOnUiThread(new Runnable() {
                ...
                public void run() {
                    ...
                    alertDialog= new AlertDialog.Builder(MeetingActivity.this).setCancelable(false)
